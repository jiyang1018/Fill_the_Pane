...
# ═══════════════════════════════════════════════════════════════════════════
#  Benchmark worker  — simulates copying a large file from RAM to storage
#  64 MB chunks, buffered I/O, no per-chunk fsync, single fsync on close
# ═══════════════════════════════════════════════════════════════════════════

CHUNK            = 64 * 1024 * 1024   # 64 MB — matches large file copy behavior
...
class BenchmarkWorker(threading.Thread):
    ...
    def _run(self):
        ...
        # Pre-allocate 64 MB random buffer in RAM once — source data lives in RAM
        ...
        try:
            # Default buffered I/O — matches OS file copy behavior
            with open(tmp, "wb") as f:
                while bytes_written < tb:
                    ...
                    # No flush/fsync per chunk — let OS manage buffering
                ...
                # Single fsync at close — same as finishing a file copy
                try:
                    f.flush()
                    os.fsync(f.fileno())
                except Exception:
...
class FillThePane(tk.Tk):
    ...
    def __init__(self):
        ...
        self._fill_min     = 10    # minimum fill %, recomputed on drive select
    ...
    def _build_left(self, p):
        ...
        btn(self._("load_data"),      self._do_load_data,        th("ACCENT")).pack(fill="x", padx=14, pady=(0, 10))
    ...
    def _on_drive_select(self, _e):
        ...
        # Compute dynamic minimum:
        # max( 10%, ceil((used + 1 GB) / total * 100) )
        used = total - free
        one_gb = 1024 ** 3
        min_pct_from_used = math.ceil((used + one_gb) / total * 100) if total > 0 else 10
        self._fill_min = max(10, min(min_pct_from_used, 89))  # never >= 90
