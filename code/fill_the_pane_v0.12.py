...
# ═══════════════════════════════════════════════════════════════════════════
#  Settings  (settings.ini next to the exe / script)
# ═══════════════════════════════════════════════════════════════════════════

def _settings_path():
    """Return path to settings.ini beside the exe or script."""
...
# ═══════════════════════════════════════════════════════════════════════════
#  Font  — Segoe UI Variable on Windows, Inter elsewhere
# ═══════════════════════════════════════════════════════════════════════════
...
class FillThePane(tk.Tk):
    ...
    def __init__(self):
        ...
        # Load settings first so theme is applied before building UI
        ...
        self._drive_total  = 0    # total drive capacity in bytes (for x axis)
        self._frame_snaps  = []
        self._vline        = None
    ...
    def _build_graph(self, p):
        ...
        # Write status line (replaces progress bar)
        self._write_status_var = tk.StringVar(value="")
        self._write_status_lbl = tk.Label(p, textvariable=self._write_status_var,
                                           font=FONT_SMALL(), fg=th("SUBTEXT"),
                                           bg=th("PANEL"), anchor="w")
        self._write_status_lbl.pack(fill="x", padx=14, pady=(4, 2))

        # 16:9 matplotlib figure — uses a container frame that resizes,
        # and we bind <Configure> to enforce the 16:9 aspect on the figure.
        self._graph_container = tk.Frame(p, bg=th("PANEL"))
        self._graph_container.pack(fill="both", expand=True, padx=10, pady=(0, 10))

        # Initial figure — sized at 16:9; actual DPI-based size doesn't matter
        # because tight_layout + canvas fill="both" will stretch it.
    ...
    def _on_graph_resize(self, event):
        """Keep figure at 16:9 by adjusting figure size to match widget width."""
        w_px = event.width
        h_px = event.height
        ...
        w_in = w_px / dpi
        h_in = w_in * 9 / 16
        # Only update if canvas height accommodates 16:9
        if h_px / dpi < h_in:
            h_in = h_px / dpi
            w_in = h_in * 16 / 9
    ...
    def _style_axes(self):
        ...
        # X axis label — shows drive capacity unit dynamically
        ...
        if self._drive_total > 0:
            ...
            if total_gb >= 1:
                ...
                total_mb = self._drive_total / (1024**2)
                x_label += f"  (MB, max {total_mb:.0f} MB)"
    ...
    def _set_theme(self, mode):
        ...
        self._build_ui()
        self._refresh_drives()
        # Restore samples to graph if any
        if self._samples:
            self._update_graph()
    ...
    def _on_drive_select(self, _e):
        ...
        self._info_labels["Total"].config(text=_fmt_size(total), fg=th("TEXT"))
        ...
        # Update x-axis label with drive capacity info
        self._ax.cla()
        self._style_axes()
        self._canvas.draw_idle()
    ...
    def _update_write_status(self, pct, bw, total, mb_s):
        """Text-only write progress — no progress bar."""
        self._write_status_var.set(
            f"{_fmt_size(bw)} / {_fmt_size(total)}  ({pct:.1f}%)   {mb_s:.1f} MB/s"
        )

    def _x_val(self, bytes_written):
        """Convert bytes written → x-axis value (GB or MB of total drive capacity)."""
        if self._drive_total <= 0:
            return bytes_written / (1024**3)
        total_gb = _bytes_to_gb(self._drive_total)
        if total_gb >= 1:
            return _bytes_to_gb(bytes_written)          # GB
        else:
            return bytes_written / (1024**2)             # MB

    def _x_label(self):
        if self._drive_total <= 0:
            return self._("capacity_axis") + " (GB)"
        total_gb = _bytes_to_gb(self._drive_total)
        unit = "GB" if total_gb >= 1 else "MB"
    ...
    def _x_max(self):
        ...
        if total_gb >= 1:
            return total_gb
        return self._drive_total / (1024**2)
    ...
    def _do_export_animation(self):
        ...
        dlg = ExportDialog(self, self._("export_anim_menu"),
                           ["GIF", "MP4"], "fill_the_pane_anim", self._lang)
        self.wait_window(dlg)
        if not dlg.result:
            return
        r   = dlg.result
        ...
        if fmt == "gif":
            ...
            try:
                ...
                fd  = [(self._x_val(s[2]), s[1]) for s in self._samples]
                xmx = self._x_max() or (max(f[0] for f in fd) * 1.05)
                ...
                ax2.set_xlim(0, xmx)
                ax2.set_ylim(0, max(f[1] for f in fd) * 1.2)
                ax2.set_xlabel(self._x_label(), color=th("SUBTEXT"), fontsize=9)
                ...
                ani = manim.FuncAnimation(fig2, _anim, frames=len(fd), interval=100, blit=True)
